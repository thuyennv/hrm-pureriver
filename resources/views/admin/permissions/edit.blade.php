...
@section('main-content')
    <div class="panel-heading">
       <h4>{{ trans('admin/general.update_info') . ' ' . trans('admin/general.modules.permissions') }}</h4>
    </div>
	<div class="panel-body">
		<form class="form-horizontal" method="post" action="{{ url('/admin/permissions', ['id' => $permission->id]) }}">
			<div class="form-group {{ $errors->has('display_name') ? 'has-error' : '' }}">
				<label for="display_name" class="col-sm-3 control-label">{{ trans('form.perm_name') }} <b class="text-danger">*</b></label>
				<div class="col-sm-6">
					<input type="display_name" class="form-control" id="display_name" name="display_name" placeholder="{{ trans('form.perm_name') }}" value="{{ Request::old('display_name', $permission->display_name) }}" />
			...
			<div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
				...
				<div class="col-sm-6">
					<input type="text" class="form-control" id="name" name="name" placeholder="{{ trans('form.perm_key') }}" value="{{ Request::old('name', $permission->name) }}" />
			...
			<div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
				...
				<div class="col-sm-6">
					<textarea class="form-control" id="description" name="description" placeholder="{{ trans('form.description') }}">{{ Request::old('description', $permission->description) }}</textarea>
